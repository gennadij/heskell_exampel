...
street :: [House]
street = [ House 1 (Cat "George" Siamese 10)
...
findOldestCat house = maybeOldestCat
    where 
        maybeOldestCat = 
            case findOldestCatHouse house of
...
findOldestCatHouse houses = 
    ...
    where housesSortedByCatAge 
            = L.sortBy catAgeComparer houses
                where catAgeComparer (House _ (Cat _ _ age1)) (House _ (Cat _ _ age2)) 
                        = compare age2 age1
...
main = putStrLn oldest
    where oldest = 
            case findOldestCatHouse street of
                Nothing -> "Es gibt keine Katze"
                Just house -> "Die aelteste Katze " 
                    ++ getCatName (getCatFromHouse house)
                    ++ ", ist "
                    ++ show (getHumanAgeOfCatFromHouse house)
                    ++ "Jahre Menschenalt und wohnt  "
